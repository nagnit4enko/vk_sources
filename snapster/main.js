...
snObject.prototype = {
  ...
  parents: function(selector) { // all parents
    ...
    var elements = [], el = selector ? O.closest(this[0], selector) : this[0].parentNode;
    while(el) {
      elements.push(el);
  ...
  last: function() {
    var keys = Object.keys(this), el = this[keys[keys.length - 1]];
    return O(el);
  },
...
var vkExpand = 'VK' + O.now(), vkUUID = 0, vkCache = {};
