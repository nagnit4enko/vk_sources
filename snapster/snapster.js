...
function getRoomCover(room) {
  if (room.cover_src) {
    return {background: 'background-image: url('+room.cover_src+');', symbol: ''};
  } else {
    ...
    return {background: 'background-color: '+colors[index]+';', symbol: String(room.title).substr(0, 1)};
  }
}
...
function getDateText(time) {
  var diff = parseInt(Date.now() / 1000) - time, timeText = '';
...
function onWinResize() {
  ...
  var cont = O('.site_cont'), contW = cont.size()[0];
...
function updateHeadPos(st) {
  var headTop = Math.min(0, Math.max(-cur.headHeight, st)), st = window.scrollY;
...
function snCheckMediaUrl() {
  ...
  if (photo) {
    ...
    photoView.show(photo[0], {no_sh_link: 1, comment_id: comment_id});
    return 1;
  }
  return 0;
}
...
var _modalHelper = {}, _modalI = 0, curModal;
...
Modal.prototype = {
  ...
  updateHeaderPos: function() {
    ...
    var st = this.layout.scrollTop(), wh = window.innerHeight;
  ...
  prepareActiveTab: function(from_switch) {
    var id = this.activeTab, tab = this.tabs[id];
    if (tab.wrap) {
    ...
    if (tab.server) {
      ...
      this.getServerData({query: this.query, cache_key: 'tab_'+id}, {
        tab: tab,
...
Scrollbar.prototype = {
  ...
  sliderDown: function(e) {
    ...
    var start_pos = e.pageY, pos, _s = this, doc = O(window);
