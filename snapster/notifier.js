if (!window.snNotify) var snNotify = {
  ...
  releaseQueueKey: function(queue_name) {
    ...
    this.sendWorkerMessage('release_key', {key_data: this.queueInfo.keys[queue_name], queueInfo: this.queueInfo});
  ...
  checkQueueKeys: function() { // check using keys
    var cur_keys = JSON.parse(localStorage.getItem('_chr_keys')), _s = this;
    _s.checkedKeys = {};
...
try { stManager.done('snapster/notify.js'); } catch(e) { }
