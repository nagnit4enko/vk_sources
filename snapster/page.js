var comments = {
  add: function(e) {
    ...
    var el = O('#sn_pw_comm_txt'), text = el.val(), pid = cur.viewPhoto;
    ...
    var query = {act: 'add_comment', owner_id: exp[0], photo_id: exp[1], message: text, hash: photo_data.comment_hash};
  ...
  delete: function(id, e) {
    ...
    var hash = O('#comment_hash' + id).val(), photo_data = cur.photosData[cur.viewPhoto];
    ajax.post('snapster.php', {act: 'delete_comment', pid: cur.viewPhoto, cid: id, hash: hash}, {
  ...
  restore: function(id, e) {
    ...
    var hash = O('#comment_hash' + id).val(), photo_data = cur.photosData[cur.viewPhoto];
    ajax.post('snapster.php', {act: 'restore_comment', pid: cur.viewPhoto, cid: id, hash: hash}, {
      onDone: function(count) {
        photo_data.comments_count = count;
        photo_data.loadedComments++;
        photo_data.comments = O('#sn_pw_comments_result').val();
  ...
  loadMore: function() {
    var id = cur.viewPhoto, data = cur.photosData[id];
    ...
    ajax.post('/snapster.php', {act: 'load_comments', owner_id: exp[0], photo_id: exp[1], offset: offset}, {
  ...
  loadBeforeComments: function() {
    ...
    var exp = id.split('_');
    ajax.post('snapster.php', {act: 'load_comments', owner_id: exp[0], photo_id: exp[1], comment_id: last_id}, {
...
var likes = {
  set: function(el, obj, opts) {
    ...
    if (!opts.noOver) {
      if (parent.tt && parent.tt.show) {
        if (!cnt || cnt < 0) parent.tt.hide({fasthide: 1});
        else parent.tt.show();
      } else {
    ...
    ajax.post('/snapster.php', {act: pref+'like', owner_id: exp[0], pid: exp[1], hash: el.getAttribute('data-hash')}, {
      onDone: function(d) {
  ...
  over: function(el, obj) {
    ...
    showTooltip(el, {
      url: 'snapster.php',
      params: {act: 'like_users', owner_id: exp[0], pid: exp[1]},
  ...
  update: function(obj, cnt) {
    if (likes.waiters[obj]) {
      ...
      if (likes.waiters[obj] == 'unlike') {
        animate(el, {marginLeft: -26}, 200);
        cnt--;
      } else {
        animate(el, {marginLeft: 6}, 200);
        cnt++;
      }

      ge('likes_box_title'+obj).innerHTML = langNumeric(cnt, cur.lang.chronicle_N_liked);
      if (!cnt || cnt < 0) {
        ge('photo_likes_el'+obj).tt.hide({fasthide: 1});
      }
    }
  },
  ...
  animComplete: function() {
    ...
    setTimeout(function() {
      animate(el, {
        opacity: 0
      }, {duration: 500, transition: Fx.Transitions.easeOutCubic});
    }, 400);
  }
};
...
function morePhotos() {
  ...
  ajax.post('/snapster.php', {act: 'room', more: 1, offset: cur.photosOffset, room_id: cur.roomID}, {
    onDone: function(d, newOffset, photos_sizes) {
      cur.photosOffset = newOffset;
      if (d) {
        var t = ce('div', {innerHTML: d}), wr = ge('photos_list_res'), first = t.firstChild;
        while(first) {
...
function onWinResize() {
  ...
  var ww = window.innerWidth, wh = window.innerHeight, pageH = getSize(bodyNode)[1], wrap3 = ge('wrap3');
  ...
  var minH = 0, panel = ge('sn_room_info_wrap');
  if (panel) {
...
var search = {
  ...
  get_result: function() {
    ajax.post('snapster.php', {act: 'search', query: ''}, {
      onDone: function(d){
  ...
  go: function(q) {
    ...
    search.timeOut = setTimeout(function() {
      ...
      ajax.post('snapster.php', {act: 'search', query: q, room_id: room_id}, {
  ...
  loadHints: function() {
    return;
    //if (!vk.id) return;
    ajax.post('snapster.php', {act: 'search', recommedations: 1}, {
      onDone: function(d) {
        cur.hintsList = d;
  ...
  onScroll: function() {
    ...
    var st = window.pageYOffset, wh = window.innerHeight, dh = document.body.offsetHeight;
    if (st + wh > dh - 200) {
      search.loadMore();
    }
  },
  loadMore: function(){
    ...
    var query = cur.searchLastQuery, global_put = geByClass1('sn_search_spliter', 'search_rooms_result') ? 1 : 0;
    ajax.post('snapster.php', {act: 'search', query: query, offset: cur.searchNextFrom, global_put: global_put}, {
      onDone: function(d, nextFrom) {
        ...
        var t = ce('div', {innerHTML: d}), res = ge('search_rooms_result'), first = t.firstChild, len = 0;
        while(first) {
...
var snModal = {
  ...
  confirmBox: function(title, text, opts) {
    ...
    var show_opts = {
      buttons: [
        {str: opts.accept_str, js: opts.accept_js, blue: 1, id: 'confirm_accept_btn'},
        {str: getLang('global_close'), js: 'snModal.close()', gray: 1, id: 'confirm_close_btn'}
      ],
      size: [451, 'auto'],
      content: text,
      title: title,
  ...
  resize: function(from_queue) {
    ...
    setStyle(el, {marginTop: marginTop});
  ...
  close: function() {
    ...
    setStyle(bodyNode, {'overflow-y': 'auto'});
  ...
  closeAll: function() {
    var wrap = ge('sn_modal_wrap'), modal;
...
var photos = {
  ...
  startUpload: function(files) {
    ...
    autosizeSetup('photo_descr_val', {maxHeight: 170, onResize: snModal.resize});
    snModal.resize();
  ...
  saveComplete: function(descrHash) {
    ...
    var tmp = ce('div', {innerHTML: cur.uploadedPhotos}), parent = ge('photos_list_res');
  ...
  saveDescr: function(pid, hash) {
    ...
    ajax.post('snapster.php', {act: 'save_photo_descr', pid: pid, descr: str, hash: hash}, {
  ...
  deletePhoto: function(pid) {
    ...
    ajax.post('snapster.php', {act: 'delete_photo', pid: pid, hash: cur.photosData[pid].delete_hash}, {
      onDone: function() {
...
var videos = {
  show: function(id){
    ...
    showInlineVideo(id, '', {autoplay: 1}, false, ge('sn_video_wrap'));
    snModal.resize();
  }
};
...
var room = {
  ...
  updatePhotos: function(e, data_only) {
    ...
    var ww = window.innerWidth, is_center = false;
    if(ww > 650) {
      ww -= 120;
    }
    ww -= 240;// panel
  ...
  assign: function(items) {
    ...
    for(var i = 0; i < items.length; i++){
      ...
      var item = items[i], pid = item.id.substr(5);
  ...
  do_set_follow: function() {
    ...
    ajax.post('snapster.php', {act: act + 'follow', room_id: cur.roomID, invite_key: cur.inviteKey, hash: cur.followHash}, {
      onDone: function() {
  ...
  showFollowers: function() {
    ...
    ajax.post('snapster.php', {act: 'get_followers', room_id: cur.roomID}, {
      onDone: function(d) {
  ...
  followersSwitchTab: function(el) {
    ...
    if (id == 'authors' && !cur.authorsTabLoaded) {
      ...
      ajax.post('snapster.php', {act: 'get_followers', room_id: cur.roomID, tab: 'authors'}, {
        onDone: function(d) {
  ...
  moreFollowers: function() {
    ...
    ajax.post('snapster.php', {act: 'get_followers', room_id: cur.roomID, offset: dat.offset, tab: tab}, {
      onDone: function(d) {
        if (!d) return;
        ...
        var t = ce('div', {innerHTML: d.html}), first = t.firstChild, wrap = geByClass1('followers_result', 'followers_box_' + tab);
        while(first) {
  ...
  clickRoom: function(id, e, domain) {
    ...
    var badge_el = geByClass1('rooms_list_item_badge', 'room' + id), badge = intval(val(badge_el));
    if (badge > 0) {
  ...
  setRoom: function(e) {
    ...
    ajax.post('snapster.php', {act: 'edit_room', room_id: room_id}, {
      onDone: function(html, header, langs) {
        ...
        autosizeSetup('sn_room_descr', {maxHeight: 170, onResize: snModal.resize});
  ...
  checkFixInviteUserPanel: function() {
    var st = this.scrollTop, wh = window.innerHeight;
  ...
  onPhoto: function(img) {
    ...
    reader.onload = (function(theFile) {
      return function(e) {
        ...
        var mleft = 0, mtop = 0, wr_size = 112;
        if (width > height) {
          var h = wr_size;
  ...
  searchInviteUsers: function(e) {
    ...
    cur.searchInviteUsersTimer = setTimeout(function() {
      ...
      ajax.post('snapster.php', {act: 'search_invite_users', room_id: cur.roomID, q: q}, {
        onDone: function(d) {
  ...
  accpetInvite: function(id, e, hash) {
    ...
    cur.roomsOffset--;
    ajax.post('snapster.php', {act: 'accept_invite', room_id: id, hash: hash}, {onDone: function() { }});
  },
  rejectInvite: function(id, e, hash, target) {
    ...
    ajax.post('snapster.php', {act: 'reject_invite', room_id: id, hash: hash}, {onDone: function() { }});
  },
...
var photoView = {
  ...
  show: function(id, opts) {
    ...
    if (!opts.no_sh_link) {
      ...
      history.pushState({h: h}, false, h);
    }

    var data = cur.photosData[id];

    var imgEl = ge('sn_pw_image'), size = photoView.getBigSrc(data.sizes);
    cur.photoSize = size;
  ...
  getBigSrc: function(sizes) {
    var ww = window.innerWidth, size = 0;
  ...
  showBox: function(id, opts) {
    ...
    autosizeSetup('sn_pw_comm_txt', {maxHeight: 60, onResize: photoView.resizeImage});
  ...
  next: function() {
    ...
    var pos = photoView.getCurPos() + 1, len = cur.pwList.length;
    if (pos >= len) {
  ...
  close: function() {
    ...
    history.replaceState({h: cur.photoViewLoc}, false, cur.photoViewLoc);
  ...
  checkLike: function(e, el) {
    ...
    var heart = geByClass1('sn_pw_heart', el), data = cur.photosData[cur.viewPhoto];
    ...
    ajax.post('/snapster.php', {act: pref+'like', owner_id: exp[0], pid: exp[1], hash: hash}, {
      onDone: function(d) {
  ...
  expandDescr: function(force_close) {
    var el = ge('sn_pw_descr_cont'), wr = ge('sn_pw_descr');
  ...
  showLikesList: function(e) {
    ...
    ajax.post('snapster.php', {act: 'get_likers', pid: cur.viewPhoto}, {
      onDone: function(d) {
  ...
  moreLikers: function() {
    ...
    ajax.post('snapster.php', {act: 'get_likers', pid: pid, offset: cur.likersOffset}, {
      onDone: function(d) {
        if (!d || pid != cur.viewPhoto) return;
        cur.likersOffset = d.offset;
        cur.likersTotal = d.total;

        var t = ce('div', {innerHTML: d.html}), first = t.firstChild, wrap = ge('followers_result');
        while(first) {
          wrap.appendChild(first);
          first = t.firstChild;
        }
  ...
  loadPhotosInfo: function(photo, opts) {
    ...
    var query = {act: 'photos_info', photo: photo};
    if(opts.comment_id) {
    ...
    ajax.post('snapster.php', query, {
      onDone: function(data, list, offset, startFrom, total, room_id, extened_data) {
        ...
        if (opts.with_loader) {
          photoView.show(photo, {no_server: 1, comment_id: opts.comment_id});
        }
      }
    });
  },
  loadPrev: function(noShow, after) {
    ...
    var query = {act: 'photos_info', photo: cur.viewPhoto, room_id: cur.pwRoomID};
    if (after) {
  ...
  restorePhoto: function(e) {
    ...
    var pid = cur.viewPhoto;
    ajax.post('snapster.php', {act: 'restore_photo', pid: pid, hash: cur.photosData[pid].delete_hash}, {
      onDone: function(res) {
        if (res == 1) {
  ...
  scrollToComment: function(cid) {
    ...
    setTimeout(function() {
      animate(el, {backgroundColor: '#fff'}, 2000);
    }, 3000);
  },
};
...
function snCheckMediaUrl() {
  ...
  if (photo) {
    ...
    photoView.show(photo[1], {no_sh_link: 1, comment_id: comment_id});
    return 1;
  }
  return 0;
}
...
function checkLeftPanelFix() {
  ...
  var panel = ge('sn_room_info_wrap'), size = getSize(panel);
  var cont = ge('wrap3'), contSize = getSize(cont);
  var winH = window.innerHeight, scrollTop = window.scrollGetY();
...
var snUser = {
  edit: function() {
    ajax.post('snapster.php', {act: 'user_edit'}, {
  ...
  shangeNameCancel: function(el, e) {
    ...
    ajax.post('snapster.php', {act: 'cancel_change_name', hash: dat.hash, rid: dat.rid}, {
      onDone: function() { }
    });
  },
  save: function(hash) {
...
var snInput = {
  ...
  blur: function(el) {
    var v = O(el).val(), parent = O(el.parentNode);
...
function snDoChangeSiteVer() {
  var url = location.href, ver = is_mob ? 'full' : 'mob';
...
var moder = {
  roomTools: function(e) {
    ...
    ajax.post('snapster.php', {act: 'room_manage', room_id: cur.roomID}, {
  ...
  saveRoom: function(hash) {
    ...
    ajax.post('snapster.php', {act: 'moder_save_room', room_id: cur.roomID, hash: hash, ban: ban, domain: domain, verify: verify}, {
      onDone: function() {
        snModal.close('room_manage');
      },
      onFail: function() {
...
try{ stManager.done('snapster/page.js'); }catch(e) { }
