...
var roomPanel = {
  ...
  showFollowers: function(tab) {
    ...
    var tabs = {
      items: [
        {id: 'all', text: lang.tab_all,  wrap: '#room_all_result', server: 1, server_query: {section: 'all'}},
      ],
      active: tab,
    };
    if (roomPanel.roomID > 0) {
      tabs.items.push({id: 'publishers', text: lang.tab_authors, wrap: '#room_publishers_result', server: 1, server_query: {section: 'publishers'}});
    }
    showBox({
  ...
  unfollow: function(force) {
    if (!force && roomPanel.role != 'viewer') {
      ...
      .addButton(lang.cancel, 'curModal.close()', {inline: 1})
  ...
  prepareSuggestItems: function(data) {
    ...
    if (O.inArray(role, ['creator', 'admin'])) {
      var out_class = 'no_display', in_class = '';
    } else {
      var out_class = '', in_class = 'no_display';
    }

    var result = '';
    for(var i in data.feed) {
      var item = data.feed[i], pid_exp = item.id.split('_');
...
var roomsList = {
  ...
  prepareInvitations: function(data) {
    ...
    var rooms_list = data.rooms_list, result = '';
...
var roomView = {
  ...
  renderPublishers: function(data) {
    var authors = '', items = data.users_list;
    roomsHub.putRooms(data.rooms_data);
    for(var i in items) {
      var id = items[i];
  ...
  assignPhotos: function() {
    var max_h = 0, margin = 10, itemW = 256;
    if (isMob) {
      margin = 3;
    ...
    O('.room_view_photo_noprepared').each(function() {
      ...
      var el = O(this), ph = parseInt(el.attr('data-height')), pw = parseInt(el.attr('data-width')), h = ph / pw * itemW;
      el.setStyle({
    ...
    if (max_h) {
      O('#room_photos').setStyle({height: max_h + 'px'});
    }
    if (isMob) {
...
var photoView = {
  show: function(id, opts) {
    ...
    var data = cur.photosData[id], size = photoView.getBigSrc(data.sizes);
    cur.photoSize = size;
    ...
    if (cur.pwType == 'video') {
      ...
      var imgEl = O('#sn_pw_image'), page_photo = O('#photo'+id);
      if (page_photo.count()) {
  ...
  updateInfo: function() {
    ...
    if (me.logged) {
      ...
      comments_data.cover = getRoomCover({cover_src: 0, id: 0});
    }
  ...
  photoEditTools: function(data) {
    var tools = [], roomData = roomsHub.getRoom(data.room_id);

    /*if (data.can_edit) {
      tools.push([lang.room_edit_button, 'photoView.editPhoto()']);
    }*/
    if (data.owner_id == me.id || O.inArray(roomData.role, ['creator', 'admin'])) {
  ...
  sendSeenPhotos: function() {
    ...
    var viewed_photos = $s('viewed_photos'), list = [];
  ...
  checkLike: function(e, el) {
    ...
    var heart = el.children('.sn_pw_heart'), data = cur.photosData[cur.viewPhoto], ic = O('#sn_pw_like_ic');
    ...
    ServerRequest.send({act: pref+'like', object: cur.viewPhoto, hash: hash});
  },
...
var commentsModule = {
  ...
  deleteComment: function(el) {
    ...
    var hash = parent.children('input').val(), cid = parseInt(parent.attr('id').replace('comment', ''));
    ServerRequest.send({
      act: 'delete_comment',
      object: cur.viewPhoto,
      cid: cid,
  ...
  checkFromSize: function(el) {
    var h = el.height(), oldH = parseInt(el.attr('data-last-height'));
    if (h != oldH) {
  ...
  loadMoreComments: function() {
    ...
    var pid = cur.viewPhoto, height = cur.commentsScroll.el.scrollHeight();
    ServerRequest.send({
      act: 'load_comments',
      object: cur.viewPhoto,
  ...
  loadBeforeComments: function() {
    ...
    var pid = cur.viewPhoto, height = cur.commentsScroll.el.scrollHeight(), cid = O('#pw_comments_result .comment:last-child').attr('id').replace('comment', '');
    ServerRequest.send({
      act: 'load_comments',
      photo: cur.viewPhoto,
      comment_id: cid,
...
var Search = {
  ...
  setFollow: function(target, el, force) {
    ...
    var room_id = parseInt(el.attr('data-room-id')), roomInfo = roomsHub.getRoom(room_id);
    ...
    ServerRequest.send({
      ...
      onDone: function() {
        ...
        roomsHub.putRooms({id: room_id, role: un ? '' : 'viewer'});
      },
      onFail: function() {
        buttonLoadingStop(target);
        if (target.hasClass('button')) {
  ...
  hashTagInit: function(hashtag, from_search) {
    ...
    if (from_search) {
      ...
      history.pushState({h: href}, false, href);
    }
    if (hashtag) {
      ...
      switch(item.type) {
        case 'follow':
          ...
          if (item.room_id > 0) {
            ...
            text = text.replace('%s', makeLink(item_data.room.domain, item_data.room.title, {className: 'notify_link'}));
          } else {
            text = text.replace('%s', '');
          }
          item_data.text = text;
        ...
        case 'create_room':
          wrap_type = this.wrapTypes.ROOM;
          item_data.text = langSex(item_data.user.sex, 'notfiy_create_room').replace('%s', makeLink(item_data.room.domain, item_data.room.title, {className: 'notify_link'}));
          break;
        case 'set_room_access':
          ...
          .replace('%s', makeLink(item_data.room.domain, item_data.room.title, {className: 'notify_link'}));
          break;
        case 'suggest_photo_accepted':
        case 'suggest_photo_rejected':
          wrap_type = this.wrapTypes.PHOTO;
          item_data.user = item_data.room;
          item_data.text = lang['notify_'+item.type].replace('%s', makeLink(item_data.room.domain, item_data.room.title, {className: 'notify_link'}));
...
var roomsManage = {
  createRoom: function(room_id) {
    ...
    tabs.items.push({id: 'settings', text: lang[room_id > 0 ? 'edit_room' : 'create_room'], wrap: '#create_room_tab_settings'});

    if (!cur.createRoomVars.editMode) {
      //tabs.items.push({id: 'invite', text: lang.invite_friends, wrap: '#create_room_tab_invite'});
    } else {
      tabs.items.push({id: 'authors', text: '������', wrap: '#create_room_tab_authors'});
      tabs.items.push({id: 'viewers', text: '����������', wrap: '#create_room_tab_viewers'});
    }
  ...
  prepareInviteItems: function(list, data, tab, append) {
    ...
    var result = '';
    for(var i in list) {
      var uid = list[i], roomData = roomsHub.getRoom(-uid);

      var class_name = '', caption = '', extra = {tab: tab};
      if (tab === 'invite') {
        var tools_opts = {
  ...
  editRoleBox: function(id, el) {
    ...
    cur.editRoomRadio = snRadio('#edit_room_role_radios', {
      items: [
        {id: 'admin', text: lang.role_admin, caption: lang.role_admin_descr},
        {id: 'publisher', text: lang.role_author, caption: lang.role_publisher_descr},
        {id: 'viewer', text: lang.role_viewer, caption: lang.role_viewer_descr},
      ],
      def: role,
    });
    curModal.update();
...
var offlineActions = {
  ...
  getAuthUrl: function() {
    return 'https://oauth.vk.com/authorize?client_id=3771002&redirect_uri='+encodeURIComponent('https://'+document.domain+'/vkauth')+'&display=page&response_type=token&v=5.40&revoke=1&scope=offline';
  },
  initVkAuth: function() {
    ...
    if (!token) {
      ...
      cur.authToken = token;
      history.pushState({h: '/rooms'}, false, '/rooms');
      roomsList.init();
    }
  },
  logout: function() {
...
var Admin = {
  ...
  showRoomAdminTools: function() {
    showBox({
      ...
      tabs: {
        items: [
          {id: 'manage', text: 'Manage',  wrap: '#room_admin_manage'},
          {id: 'promo', text: 'Promo',  wrap: '#room_admin_promo'},
        ],
        active: 'manage',
...
var otherPages = {
  ...
  renderAboutPage: function(data) {
    ...
    setTitle(lang.about);
    setPageContent(Templates.get('sitePagesWrap', {about: 'top_nav_item_active'}, Templates.get('aboutPage')));
    otherPages.checkRoomPanel(data.room_info);
  },
  initTerms: function() {
    Page({
      ...
      setTitle(data.title);
      setPageContent(Templates.get('sitePagesWrap', {terms: 'top_nav_item_active'}, otherPages.prepareBlocks(data.blocks, data.title)));
      otherPages.checkRoomPanel(data.room_info);
    });
  },
