VideoEmbed = {
    ...
    getPage: function(offset) {
        ...
        ajax.post('/video_embed', {act: nav.objLoc.act, filter: nav.objLoc.filter, offset: offset, load: 1}, {
            cache: 1,
    ...
    setHostStatus: function(cid, status, onDone, sure, comment) {
        var params = {act: 'host_set_status', host_id: cid, status: status};
    ...
    setOwnerStatus: function(cid, status, onDone, sure, comment) {
        var params = {act: 'owner_set_status', owner_id: cid, status: status};
        ajax.post('/video_embed', params, {
...
_eof: 1};try{stManager.done('video_embed.js');}catch(e){}
