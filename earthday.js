...
function earthDayInit() {
  ...
  animate(earthTrigger, {opacity: 1}, {duration: 1200, transition: Fx.Transitions.easeOutCubic});
  animate(ge('earthday_lamp'), {marginTop: 0}, {duration: 300, transition: Fx.Transitions.easeOutCubic});
  animate(ge('earthday_chain'), {marginTop: -60}, {duration: 800, transition: Fx.Transitions.easeOutCubic});

  addEvent(earthTrigger, 'mouseover', function() {
    animate(ge('earthday_chain'), {marginTop: -55}, {duration: 300, transition: Fx.Transitions.easeOutCubic});
  });

  addEvent(earthTrigger, 'mouseout', function() {
    animate(ge('earthday_chain'), {marginTop: -60}, {duration: 300, transition: Fx.Transitions.easeOutCubic});
  });

  addEvent(earthTrigger, 'mousedown', function(ev) {
    animate(ge('earthday_chain'), {marginTop: -25}, {duration: 300, transition: Fx.Transitions.easeOutCubic, onComplete: function() {
      animate(ge('earthday_chain'), {marginTop: -60}, {duration: 200, transition: Fx.Transitions.easeOutCubic});
      lightTrigger();
    }});
...
function eathdayClose() {
  animate(earthdayBG, {opacity: 0}, 300, function() {
    hide(earthdayBG);
  });
...
function showBacklight() {
  ...
  if (!earthdayBG) {
    earthdayBG = ce('div', {className: 'earthday_bg', innerHTML: html}, {opacity: 0});
  } else {
    show(earthdayBG);
  }
  ...
  animate(earthdayBG, {opacity: 1}, 800);
  setStyle(ge('eathday_info'), {opacity: 1})
}
...
function earthdayLighter(ev) {
  ...
  lighterOn = true;
  animate(ge('eathday_info'), {opacity: 0}, 400, function() {
    addClass(earthdayBG, 'earthday_bg_lighter');
    setStyle(earthdayBG, {width: window.lastWindowWidth+500, height: window.lastWindowHeight+500});
    addEvent(bodyNode, 'mousemove', function(ev) {
      var x = ev.pageX;
      var y = ev.pageY - scrollGetY();
      setStyle(earthdayBG, {backgroundPosition: (x-200)+'px '+(y-200)+'px'});
    });
  });
  var x = ev.pageX;
  var y = ev.pageY - scrollGetY();
...
function lightTrigger() {
  if (lighterOn) {
    ...
    setStyle(earthdayBG, {backgroundPosition: '0px 0px', width: '100%', height: '100%'});
...
try{stManager.done('earthday.js');}catch(e){}
