var blog = {
  ...
  initBack: function () {
    cur._back = {loc: false, show: [blog.bindEvents], hide: [blog.destroy], text: 'Blog'};
  },
  bindEvents: function () {
    blog.destroy();
    if (cur.nextFrom) {
  ...
  onScroll: function() {
    ...
    var st = scrollGetY(), bh = getSize(bodyNode)[1], wh = lastWindowHeight;
    if ((st + wh) * 2 >= bh) {
      blog.loadMore();
    }
  },
  loadMore: function() {
    ...
    ajax.post('blog.php', {
      ...
      onDone: function(rows, nextFrom) {
        ...
        var first = tmp.firstChild, wrap = ge('blog_posts_list');
        while(first) {
        ...
        if (cur.nextFrom) {
          ...
          var more_btn = ge('blog_more_but');
          more_btn && re(more_btn.parentNode);
        }
      },
      onFail: function() {
        unlockButton('blog_more_but');
  ...
  checkEntryVideos: function() {
    ...
    showInlineVideo(matches[1], matches[2], {autoplay: 1}, false, video);
  },
  ...
  checkCanDragCover: function() {
    var wrapH = Math.floor(getSize('blog_cover_wrap')[1]), imgH = Math.floor(getSize('blog_cover_img')[1]);
    if (imgH > wrapH) {
  ...
  uploadCover: function(callback) {
    ...
    img.onload = function() {
      ...
      var img_h = parseInt(size[1] / size[0] * cW);

      ctx.drawImage(img, 0, ge('blog_cover_img').offsetTop * 2, cW, img_h);
  ...
  save: function() {
    ...
    var title = trim(val('blog_entry_title')),
      ...
      lang_id = cur.blogLangDD.val(),
      adres = trim(val('blog_entry_adres')),
      author_id = cur.blogAuthorDD.val(),
    ...
    if (title.length == 0) {
      return ge('blog_entry_title').focus();
    }
    if (text.length == 0) {
    ...
    if (adres.length == 0 || !adres.match(/^([a-zA-Z0-9\-\_]+)$/)) {
      showDoneBox(cur.lang.blog_bad_entry_link, {out: 4000});
    ...
    var query = {
      ...
      title: title,
      Message: text,
      Short: trim(val('blog_entry_short_text')),
      lang_id: lang_id,
      adres: adres,
      author_id: author_id,
      cat: cat,
      ...
      onDone: function(code) {
        switch(code) {
          case 'adres':
            ...
            showDoneBox(cur.lang.blog_bad_adres, {out: 4000});
            break;
          default:
            if (cur.entryID > 0) {
  ...
  toTranslit: {1072:"a",1073:"b",1074:"v",1075:"g",1076:"d",1077:"e",1078:"zh",1079:"z",1080:"i",1081:"y",1082:"k",1083:"l",1084:"m",1085:"n",1086:"o",1087:"p",1088:"r",1089:"s",1090:"t",1091:"u",1092:"f",1093:"h",1094:"ts",1095:"ch",1096:"sh",1097:"sh",1099:"y",1101:"e",1102:"yu",1103:"ya", 1105:"e", 1098:"", 1100:""},
  translitTitleToAdres: function(str) {
    ...
    str = str.toLowerCase();
    var adres = '', exp = str.split('');
    for(var i in exp) {
      var code = str.charCodeAt(i), symb = '';
      if (code == 32) {
        symb = '-';
  ...
  doRemove: function() {
    ajax.post('blog.php', {act: 'remove', id: cur.entryID, hash: cur.blogHash}, {onDone: function() {
      nav.go('/blog');
    }});
  },
  setFollow: function(action, hash) {
    ...
    ajax.post('blog.php', {act: act + 'follow', hash: hash}, {
      onDone: function(res, data){
        if (action == 'follow') {
          unlockButton('blog_follow_btn');
        }
        if (res == 'confirm_email') {
          var mbindBox = MessageBox({title: data.title, onHide: function() { }}).content(data.html);
  ...
  showEmailSettings: function(unfollow_hash) {
    showBox('blog.php', {
      act: 'email_settings',
    })
    .removeButtons()
    ...
    .setControlsText('<a href="/" onclick="blog.setFollow(\'unfollow_box\', \''+unfollow_hash+'\'); return false;">'+cur.lang.blog_unfollow_box+'</a>');
  },
  ...
  toYear: function(year, e) {
    ...
    if (el) {
      var pos = getXY(el), top = Math.max(0, pos[1] - 20 - getSize('page_header_cont')[1]);
      if (browser.msie6) {
        animate(pageNode, {scrollTop: top}, 400);
      } else {
        animate(htmlNode, {scrollTop: top}, {duration: 800, transition: Fx.Transitions.easeOutCubic});
        animate(bodyNode, {scrollTop: top}, {duration: 800, transition: Fx.Transitions.easeOutCubic});
      }
    }
  },
  ...
  notificationSave: function (btn) {
    ...
    if (platformsList.length == 1 && platformsList[0] != 'web2' || platformsList.length > 1) {
      var photo_id = attr('blog_notification_image', 'data-pid');
      if (!photo_id) {
      ...
      var button_text = trim(val('notification_button_text'));
      if (!button_text) {
    ...
    var groups = [], groupsRaw = cur.groupsSel.val_full();
  ...
  notificationOnChangePlatform: function () {
    ...
    if (cur.mobPlatform) {
      show('notification_type_wrap');
    } else {
      hide('notification_type_wrap');
    }
  ...
  notificationDoSelectImage: function (photo_raw, photo_url, ratio) {
    curBox().hide();
    setStyle('blog_notification_image', 'background-image', 'url(' + photo_url + ')');
    attr('blog_notification_image', 'data-pid', photo_raw);
...
try{ stManager.done('blog.js'); } catch(e) { }
